...
| Server | Rule |
|--------|------|
| `SERVER_PROFILE_1` | **Full Access** on `extend_db_ptrj`, READ-ONLY on others |
| Others (`readOnly: true`) | **SELECT only** - Write operations will return 403 Forbidden |

### Database-Level Permissions

| Database | Permissions |
|----------|-------------|
| `extend_db_ptrj` | **FULL ACCESS** (SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, etc.) - *Only on SERVER_PROFILE_1* |
| `db_ptrj` | **READ-ONLY** (SELECT only) |
| Other databases | **READ-ONLY** (SELECT only) |
...
```
*Note: Operations above are ONLY allowed on `extend_db_ptrj` using `SERVER_PROFILE_1`.*
