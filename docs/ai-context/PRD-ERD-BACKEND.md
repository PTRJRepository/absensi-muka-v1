# PRD: ERD + Backend Database — Monitoring Absensi Muka PT Rebinmas Jaya

## Project Root
`D:/Gawean Rebinmas/Absensi_Muka/`

## Tech Stack
- SQL Server (via HTTP Gateway: `http://10.0.0.110:8001/v1/query`, server `SERVER_PROFILE_1`, database `extend_db_ptrj`)
- Node.js v22.14.0
- TypeScript
...
```
MESIN/API → RAW IMPORT (immutable) → NORMALIZATION → PROCESS → RECONCILIATION → MONITORING
```

## Implementation Order

1. Master tables: `mst_estate`, `mst_division`, `mst_gang`, `mst_machine`, `mst_employee`
2. Movement tracking: `employee_division_history`, `employee_daily_assignment`
3. Import layer: `sync_job`, `import_batch`
4. Raw data: `attendance_raw_log`, `machine_user_raw`, `api_attendance_raw`
5. Mapping: `machine_user_map`
6. Process: `attendance_daily_process`, `attendance_process_detail`
7. Reconciliation: `attendance_division_reconcile`
8. Manual: `attendance_manual_adjustment`
9. Monitoring: `attendance_anomaly`, `monitoring_daily_summary`
10. System: `audit_log`, `app_config`
11. Indexes
12. Views: `vw_attendance_monitoring_daily`, `vw_attendance_anomaly_open`
13. Seed data: 15 machines + 13 divisions
14. TypeScript backend skeleton: `src/modules/` structure

## Full Schema & Details

See the complete PRD below for all table definitions, constraints, indexes, views, and seed data.
...
Sample estates: PGE, DME, ARA, AB1, AB2, ARC, IJL, MILL, ARE
...
Mapping:
- P1A/PG1A → scanner 100 → loc A
- P1B/PG1B → scanner 300 → loc B
- P2A/PG2A → scanner 500 → loc C
- P2B/PG2B → scanner 600 → loc D
- DME → scanner 700 → loc E
- ARA → scanner 800 → loc F
- AB1/ARB1 → scanner 900 → loc G
- AB2/ARB2 → scanner 400 → loc H
- ARC/AREC → scanner 200 → loc J
- IJL → loc L
...
SELECT p.work_date, e.emp_code, e.emp_name, d.division_code, d.division_name, g.gang_code,
    p.jam_masuk, p.jam_keluar, p.scan_count,
    p.has_machine_log, p.has_api_data, p.has_manual_adjustment,
    p.has_work, p.is_cuti, p.is_sakit, p.is_holiday, p.ot_hours, p.task_code,
    p.attendance_status, p.reconcile_status,
    r.match_status, r.mismatch_reason, p.processed_at
...
SELECT a.anomaly_id, a.detected_at, a.work_date, e.emp_code, e.emp_name,
    d.division_code, m.machine_code, a.anomaly_type, a.severity, a.title, a.description, a.status
...
## 16. Seed Data — 15 Machines

```sql
INSERT INTO mst_estate (estate_code, estate_name) VALUES
('PGE', 'Estate PGE'), ('DME', 'Estate DME'), ('ARA', 'Estate ARA'),
('AB', 'Estate AB'), ('ARC', 'Estate ARC'), ('IJL', 'Estate IJL'),
('MILL', 'Mill'), ('ARE', 'Estate ARE');

INSERT INTO mst_division (division_code, division_name, loc_code, scanner_code) VALUES
('PG1A', 'Divisi PG1A', 'A', 100), ('PG1B', 'Divisi PG1B', 'B', 300),
('PG2A', 'Divisi PG2A', 'C', 500), ('PG2B', 'Divisi PG2B', 'D', 600),
('DME', 'Divisi DME', 'E', 700), ('ARA', 'Divisi ARA', 'F', 800),
('ARB1', 'Divisi ARB1', 'G', 900), ('ARB2', 'Divisi ARB2', 'H', 400),
('AREC', 'Divisi AREC', 'J', 200), ('IJL', 'Divisi IJL', 'L', NULL),
...
## 17. TypeScript Backend Skeleton

Create `src/modules/` structure:
```
src/
├── modules/
│   ├── machines/          # machine.repository.ts, machine.service.ts
│   ├── import/            # import-job.service.ts, direct-zkteco-import.service.ts, api-attendance-import.service.ts
│   ├── employees/         # employee.repository.ts, employee-mapping.service.ts, employee-movement.service.ts
│   ├── attendance/        # attendance-raw.repository.ts, attendance-process.service.ts, attendance-reconcile.service.ts
│   ├── monitoring/        # anomaly.service.ts, summary.service.ts, dashboard.service.ts
│   └── audit/             # audit.service.ts
├── shared/
│   ├── database/          # sql-client.ts
...
## Important Notes
- All SQL must be executed via SQL Gateway: `http://10.0.0.110:8001/v1/query` (POST, `x-api-key` header)
- Server: `SERVER_PROFILE_1`, Database: `extend_db_ptrj`
- Use SQL Gateway for all DDL and DML
- Write the complete SQL migration script first, then execute it
- Write TypeScript skeleton files after schema is confirmed
- Create `migrations/` directory for SQL files
- Create `src/` directory for TypeScript backend
- Do NOT hardcode API keys — reference `_dev_utils/src/config.ts`
