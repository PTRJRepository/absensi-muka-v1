...
-- =============================================================================
-- Update attendance view for multi-format JOIN
-- Now supports BOTH:
-- 1. parsed_employee_code = employee_code (ZKTeco format: A10041, L10086)
-- 2. raw_device_user_id = employee_code (IT API format: 0010001)
-- =============================================================================

PRINT '=== Step 2: Update view for multi-format JOIN ===';
...
INNER JOIN employees e ON (
  -- ZKTeco format (e.g., A10041, L10086)
  e.employee_code = s.parsed_employee_code
  -- OR IT API format (e.g., 0010001)
  OR e.employee_code = s.raw_device_user_id
)
INNER JOIN divisions d ON d.id = e.division_id
LEFT JOIN gangs g ON g.id = e.gang_id
WHERE e.is_active = 1
GROUP BY YEAR(s.scan_date), MONTH(s.scan_date),
     e.employee_code, e.employee_name, d.division_code, g.gang_code;
GO

-- Also update daily summary view
...
-- Also update final attendance view
...
PRINT 'Views updated with multi-format JOIN!';
GO
